...
```bash
./build.sh
docker build -f dist/templates/Dockerfile -t starbot:4.1.0-beta2 dist/build
docker run -d --name starbot --restart unless-stopped \
  -v starbot-data:/app -p 127.0.0.1:7827:7827 starbot:4.1.0-beta2
```
